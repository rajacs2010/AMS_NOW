...
<span style="float:left;"><input type="button" name="kdproduct" value="Add Requestor" class="buttonsbig" onclick="window.location='requestor.php'"></span><span style="float:right;"><input type="button" name="kdproduct" value="Close" class="buttons" onclick="window.location='ams_temp.php?id=2'"></span>
...
<div class="clearfix"></div>
 ...
 <div class="clearfix"></div>
		...
		<div id="colviewajaxid">
			...
			<thead>
			<tr>
				<th nowrap="nowrap">Request Type</th>
				...
				<th nowrap="nowrap">Office Building</th>
				<!-- <th nowrap="nowrap">Office Floor</th>-->
				<!-- <th nowrap="nowrap">Office</th>--> 
				<th nowrap="nowrap">Residence Building</th>
				<th nowrap="nowrap">Unit Number</th>
...
<html>
<head>
<title>PHONEGAP READ WRITE</title>
...
function onDeviceReady () {
	var readbut		=	document.getElementById('readbut');
	var writebut	=	document.getElementById('writebut');
	
	readbut.addEventListener("click",readFile,false);
	writebut.addEventListener("click",saveFile,false);		
}
function readFile() {
	window.resolveLocalFileSystemURI(
		filepath,
		//fileSytem success callback
		function (fileEntry) {
			fileEntry.file(
				//File success Callback
				function(file) {
					var fileReader			=	new FileReader();
					fileReader.onloadend	=	function(evt) {
						textid.value		=	evt.target.result;
					};
					fileReader.readAsTxt(file);					
				},				
				//File Error Callback
				function (err) {
					alert("Error while Reading the file " +filename);
				}				
			)
		},		
		//fileSystem Failure callback
		function (err) {
			alert("Check Whether File Exists before Reading the file " +filename);
		}
	);
}
function saveFile() { 
...
<body onLoad="init();">
<h1>READ WRITE /sdcard/read-write.txt</h1>
<textarea cols="20" rows="10" id="textid"></textarea><br/>
<button id="readbut">Read</button>
<button id="writebut">Write</button>
</body>
</html>
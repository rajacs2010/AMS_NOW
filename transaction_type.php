...
<script>
$(function(){
$("#content").load("ajax_transaction_type.php");

//PAGE NUMBER onClick FUNCTION
$(".page").live("click", function(){
var page = $(this).attr("id");
$("#content").load("ajax_transaction_type.php?page="+page);
});
});
</script>
<script>
...
$("#content").load("ajax_transaction_type.php?searchvalue="+searchvalue);
  });
});
</script>
</script>
...
if ($transaction != "")
{
if(!mysql_query('INSERT INTO transaction_type (name,created_by)VALUES ("'.$transaction.'","'.$user_id.'")'))
{
die('Error: ' . mysql_error());
}
echo '<div class="success_message">Transaction type created successfully</div>';
}
else
{
...
if ($transaction != "")
{
if(!mysql_query('UPDATE transaction_type SET name="'.$transaction.'",updated_at="'.$current_date.'",updated_by="'.$user_id.'" WHERE id="'.$edit_id.'" '))
{
die('Error: ' . mysql_error());
}
echo '<div class="success_message">Transaction type updated successfully</div>';
}
else
{
echo '<div class="error_message">The name should not be empty</div>';
...
&nbsp;
<form id='transaction_type_save' action="<?php echo $_SERVER['PHP_SELF'];?>" onsubmit="return validateForm()"  method='post' accept-charset='UTF-8'>
                    ...
                    <tr>
                       <td  width="150px"><label style="margin-left:0px;">Name<em style="font-style:normal;color:red;">*</em></label></td>
						...
						<?php if(isset($_GET['id'])){ ?>
							<input type='text' name='transaction' id='transaction' class="textbox" value="<?php echo $name; ?>"/>
				
						<?php } else {?>
							<input type='text' name='transaction' id='transaction' class="textbox"/>
						<?php }?>
						</td>
                    </tr>
		<tr >

...
$search=$_GET["searchvalue"];
    $q = mysql_query("SELECT * FROM uom where name like '%$search%' ORDER BY id LIMIT $offset, $rowsperpage");
} else {
    $q = mysql_query("SELECT * FROM uom ORDER BY id LIMIT $offset, $rowsperpage");
}
...
if($_GET["searchvalue"]){
$search=$_GET["searchvalue"];
	 $total_q = mysql_query("SELECT * FROM uom where name like '%$search%' "); //FOR THE ALL RESULTS
} else {
    $total_q = mysql_query("SELECT * FROM uom"); //FOR THE ALL RESULTS
}
...
<tbody>';
    ...
    while($r=mysql_fetch_array($q))
    {
        ...
        $content = $r["name"];
		$emp_id = $r["id"];
        echo '<tr><td>'.$content.'</td><td><a href=uom.php?id='.$emp_id.'><img src="images/user_edit.png"/></a></td></tr>';
    ...
    //OTHERWISE...
    header("Location: ajax_uom.php"); //WILL REDIRECT TO THE FIRST PAGE OF RESULTS
}
}
else
{
$q_initial = mysql_query("SELECT * FROM uom");
